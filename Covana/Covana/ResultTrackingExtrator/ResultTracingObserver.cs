...
namespace Seqex.ResultTrackingExtrator
{
    public sealed class ResultTracingObserver
...
//                    Dump("changed signature: " + fullNameWithParameters);
            ...
            private List<string> extractMethods(IEnumerable<object> exceptionObjects, StringBuilder methodsInExceptionBuilder, List<string> methodsInException, List<string> lines)
            {
                ...
                foreach (var exceptionObject in exceptionObjects)
                {
                    ...
                    while (stackTrace.IndexOf("at") != -1)
                    {
                        int atIndex = stackTrace.IndexOf("at");
                        stackTrace = stackTrace.Substring(stackTrace.IndexOf("at"));
                        atIndex = stackTrace.IndexOf("at");
                        int leftIndex = stackTrace.IndexOf("(");
                        string methodName = stackTrace.Substring(atIndex+3,leftIndex - atIndex - 3);
                        ...
                        Dump("methodName: " + methodName);
                        Dump("stacktrace: " + stackTrace);
                        Dump("line: " + stackTrace.IndexOf("line"));
                        Dump("at: " + stackTrace.IndexOf("at"));
                        if (stackTrace.IndexOf("line") < stackTrace.IndexOf("(") && stackTrace.IndexOf("at") != -1)
                        {
                            lines.Add(stackTrace.Substring(stackTrace.IndexOf("line") + 5, stackTrace.IndexOf("at") - stackTrace.IndexOf("line") - 10));
...
//                        Dump("methods: " + methodsInExceptionBuilder.ToString());
                        exceptionStrings.Add(exception.StackTrace);
                    }
                }
